...
const ALL_SECTIONS = ['parts', 'iodevices'];
const STORE_PREFIX = 'itfun:m4:';
...
/* ────────────────────────────────────────────
   Flip Card — text front, image back — with tracking
─────────────────────────────────────────────*/
...
// Total interactive items per lesson
const LESSON_TOTALS = {
  parts:     19,  // 4 hw + 1 fc(building) + 6 pc + 4 mobo + 3 cpu + 1 fc-img-back(brand)
  iodevices: 16,  // 1 fc(keyboard) + 5 kb + 1 fc(mouse) + 1 fc(monitor) + 1 fc(impact) + 2 char + 2 line + 1 fc(nonimpact) + 2 non
};
...
function Chapter4() {
  ...
  const ioTracker     = useProgressTracker(moduleId, 'iodevices', LESSON_TOTALS.iodevices);

  const trackers = { parts: partsTracker, iodevices: ioTracker };
  ...
  const progress = {
    parts:     Math.round(partsTracker.progress),
  ...
  /* ── PC Building reasons ── */
  const pcReasons = [
    { title: 'Customization and Control',   desc: 'This allows tailoring performance for specific needs.' },
    { title: 'Better Value for Money',       desc: 'By sourcing components individually, you can allocate your budget efficiently, prioritizing parts that matter most for your intended workload.' },
    { title: 'Upgradability',               desc: 'Instead of replacing the entire system, you can swap specific parts like the GPU or storage drive.' },
    { title: 'Performance Optimization',    desc: 'Match components for maximum efficiency and eliminate performance bottlenecks. Overclocking, custom cooling, and optimized airflow designs are also possible.' },
    { title: 'Educational Value',           desc: 'Building a PC deepens understanding of computer hardware. This hands-on knowledge is valuable for IT professionals, hobbyists, and students alike.' },
    { title: 'Personal Satisfaction',       desc: "There's a unique pride in powering on a machine you've assembled yourself and watching it work flawlessly." },
  ];

  /* ── Motherboard manufacturers ── */
  const moboMakers = [
    { title: 'ASUS',     desc: 'One of the top-tier brands, known for quality, stability, and innovation. Usually more expensive than competitors. Best for: Gamers, overclockers, and long-term PC builders.' },
    { title: 'Biostar',  desc: 'Budget-friendly. Very affordable compared to ASUS/MSI/Gigabyte. Fewer features (weak BIOS, basic VRM design). Less reliable for heavy gaming/overclocking. Best for: Office PCs, light-duty builds, budget gamers.' },
    { title: 'Gigabyte', desc: "Another top-tier brand, rival to ASUS. BIOS updates sometimes slower. Good gaming and enthusiast boards (AORUS line is popular). Best for: Gamers who want performance without ASUS's premium price." },
    { title: 'MSI',      desc: 'Strong presence in gaming boards. User-friendly BIOS, easy tuning features. High-end boards have strong support for overclocking. Historically weaker customer support than ASUS/Gigabyte. Best for: Gamers who like clean design and straightforward overclocking.' },
  ];

  /* ── CPU components ── */
  const cpuComponents = [
    { title: 'Memory or Storage Unit',       desc: 'This unit can store instructions, data and intermediate results. Also known as internal storage unit, main memory, primary storage, or RAM. Its size affects speed, power and capability. Functions: stores all data and instructions required for processing; stores intermediate results of processing.' },
    { title: 'Control Unit',                 desc: 'Controls the operations of all parts of the computer but does not carry out any actual data processing. Responsible for controlling transfer of data and instructions among other units. Obtains instructions from memory, interprets them, and directs the operation of the computer. Does not process or store data.' },
    { title: 'Arithmetic Logic Unit (ALU)', desc: 'Consists of two subsections — Arithmetic Section: performs operations like addition, subtraction, multiplication, and division. Logic Section: performs operations such as comparing, selecting, matching and merging of data.' },
  ];

  /* ── Ports ── */
  const ports = [
    { name: 'Serial Port',   desc: 'Used for external modems and older computer mouse. Data travels at 115 kilobits per second.',                                     img: serialImg },
    { name: 'Parallel Port', desc: 'Used to connect printers, scanners and other devices. Also called LPT port (Line Printer Terminal). Data travels at 50–100 kilobits per second.', img: parallelImg },
    { name: 'PS/2 Port',     desc: 'Used to connect older keyboards and mice. Comes in two colors: purple (keyboard) and green (mouse). Often replaced by USB.',          img: ps2Img },
    { name: 'USB Port',      desc: 'Universal Serial Bus. Most common modern port. Used for flash drives, phones, keyboards, mice, printers and more. Supports hot-swapping.', img: usbImg },
    { name: 'VGA Port',      desc: 'Video Graphics Array. Connects monitors and projectors. Analog signal. Being replaced by HDMI and DisplayPort in modern computers.',    img: vgaImg },
    { name: 'Ethernet Port', desc: 'Used to connect computers to a network or the internet via an Ethernet cable. Also called RJ-45 port. Supports speeds up to 10 Gbps.', img: ethernetImg },
  ];
            ...
            <>
              ...
              <div className="s2-body-block">
                <p className="s2-body-text">
                  A computer is made up of multiple components that work together to perform tasks. Understanding each part helps you build, maintain, and troubleshoot computers effectively.
                </p>
              </div>
              ...
              <div className="chap-accordion" style={{ marginTop: 12 }}>
                {hardwareDropdowns.map((item, i) => (
                  <AccordionItem
                    ...
                    itemId={`hw_${i}`}
                    onInteract={partsTracker.trackInteraction}
                  >
            ...
            <>
              ...
              <div className="s3-flipcard-single">
                <FlipCard
                  ...
                  itemId="fc_keyboard"
                  onInteract={ioTracker.trackInteraction}
                />
              </div>
              ...
              <div className="s2-body-block" style={{ marginTop: 12 }}>
                <p className="s2-body-text">Printer is an output device used to print information on paper.</p>
                <p className="s2-body-text">There are two types of printers:</p>
                <ul className="s3-bullet-list">
              ...
              <div className="s2-body-block" style={{ marginTop: 12 }}>
                <p className="s2-body-text">Impact printers are of two types:</p>
                <ul className="s3-bullet-list">
              ...
              <div className="chap-accordion" style={{ marginTop: 8 }}>
                {[
                  { title: 'Dot Matrix Printer (DMP)', desc: 'One of the most popular printers in the market. Characters are printed in a pattern of dots using a matrix of pins (5×7, 7×9, 9×7 or 9×9). Advantages: Inexpensive, widely used, can print other language characters. Disadvantages: Slow speed, poor quality.' },
                  { title: 'Daisy Wheel',              desc: 'Head is on a wheel with pins like petals of a daisy flower. Generally used for word-processing in offices requiring nice quality letters. Advantages: More reliable than DMP, better quality, fonts can easily be changed. Disadvantages: Slower than DMP, noisy, more expensive than DMP.' },
                ].map((item, i) => (
                  <AccordionItem
                    key={i}
                    title={item.title}
              ...
              <div className="chap-accordion" style={{ marginTop: 8 }}>
                {[
                  { title: 'Drum Printer',  desc: 'Drum-shaped printer where the surface is divided into tracks equal to the paper width. A character set is embossed on each track. One rotation of the drum prints one line. Speed: 300–2000 lines per minute. Advantages: Very high speed. Disadvantages: Very expensive, character fonts cannot be changed.' },
                  { title: 'Chain Printer', desc: 'Uses a chain of character sets. Standard character sets may have 48, 64, or 96 characters. Advantages: Character fonts can easily be changed, different languages can be used. Disadvantages: Noisy.' },
                ].map((item, i) => (
                  <AccordionItem
                    key={i}
                    title={item.title}
              ...
              <div className="chap-accordion" style={{ marginTop: 14 }}>
                {[
                  { title: 'Laser Printers',  desc: 'Non-impact page printers that use laser lights to produce dots for characters. Advantages: Very high speed, very high quality output, good graphics quality, support many fonts and character sizes. Disadvantages: Expensive, cannot produce multiple copies in a single printing.' },
                  { title: 'Inkjet Printers', desc: 'Print characters by spraying small drops of ink onto paper. Produce high quality output with less noise. Color printing is possible. Some models can produce multiple copies. Advantages: High quality printing, more reliable. Disadvantages: Expensive (high cost per page), slow compared to laser printers.' },
                ].map((item, i) => (
                  <AccordionItem
                    key={i}
                    title={item.title}
