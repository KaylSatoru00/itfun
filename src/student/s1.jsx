            ...
            <div style={{ padding: '16px 20px', background: '#fff' }}>
              {Array.isArray(description)
                ? description.map((para, i) => {
                    ...
                    return <p key={i} style={{ fontSize: 16, color: isLabel ? '#A50034' : '#333', fontWeight: isLabel ? 'bold' : 'normal', lineHeight: 1.7, margin: i > 0 ? '6px 0 0' : 0, paddingLeft: isBullet ? '8px' : 0 }}>{para}</p>;
                  })
...
const sectionData = {
  introduction: [
    { title: 'Input', description: ['The process of entering data into the computer.', 'Examples:', '• Typing using a keyboard', '• Clicking using a mouse', '• Scanning documents'], flipImage: pic1, flipText: 'A computer is an electronic device that accepts data, processes it according to a set of instructions (program), stores the data, and produces meaningful information as output.' },
    { title: 'Processing', description: ['The computer manipulates data and performs calculations.', 'Example:', "• Calculating a student's average grade."], flipImage: null, flipText: 'The CPU processes the input data by executing instructions from software programs.' },
    { title: 'Storage', description: ['The computer saves data for future use.', 'Examples:', '• Hard Disk Drive (HDD)', '• Solid State Drive (SSD)'], flipImage: null, flipText: 'Data can be stored temporarily in RAM or permanently in hard drives, SSDs, or cloud storage.' },
    { title: 'Output', description: ['The computer presents processed information.', 'Examples:', '• Monitor display', '• Printed reports', '• Audio from speakers'], flipImage: null, flipText: 'Processed data is displayed or sent to output devices like monitors, printers, or speakers.' },
  ],
};
...
function Chapter1() {
  ...
  // ── Progress trackers — one per lesson ──
  const intro = useProgressTracker('module1', 'lesson1', LESSON_TOTALS.introduction);
  const func  = useProgressTracker('module1', 'lesson2', LESSON_TOTALS.functionalities);
  const hist  = useProgressTracker('module1', 'lesson3', LESSON_TOTALS.history);

  const trackerMap = { introduction: intro, functionalities: func, history: hist };
  const activeTracker = trackerMap[activeSection];

  const progress = {
    introduction: intro.progress,
    functionalities: func.progress,
              ...
              <div className="chap-adv-section">
                ...
                <div className="chap-accordion">
                  {[
                    ...
                    <AccordionItem key={i} title={item.title} description={item.desc} isOpen={advOpenIndex === i} onToggle={() => setAdvOpenIndex(advOpenIndex === i ? null : i)} itemId={`func_adv_${i}`} onInteract={func.trackInteraction} />
                  ))}
                </div>
              </div>
            ...
            <>
              <div className="chap-history-section">
                ...
                <div className="chap-history-block">
                  ...
                  <p className="chap-history-body">One such numbering system is the decimal numbering system, which is traceable to early Hindu-Arabic influence. This decimal system with specific digits representing numbers from 0 to 9, came into general use in Europe and has survived until the present.</p>
                </div>

                <HistoryPersonBlock name="Blaise Pascal" label="— French mathematician and inventor of one of the first mechanical calculating machines." image={blaiseImg}
                  ...
                  inventions={[{ title: 'Pascaline', image: pascalineImg, description: "The Pascaline (also known as Pascal's calculator or arithmetic machine) is one of the world's first mechanical calculators. Invented by the 19-year-old French mathematician Blaise Pascal in 1642, it was designed to automatically perform addition and subtraction." }]}
                  openIndex={pascalOpenIndex} setOpenIndex={setPascalOpenIndex}
                  personItemId="hist_person_pascal" inventionBaseId="hist_inv_pascal" onInteract={hist.trackInteraction} />

                <HistoryPersonBlock name="Gottfried Wilhelm Von Leibniz" label="— German mathematician who improved Pascal's machine into a Four Functions Machine." image={leibnizImg}
                  ...
                  inventions={[{ title: 'Stepped Reckoner', image: steppedreckImg, description: 'The Stepped Reckoner was a pioneering mechanical calculator invented by the German mathematician Gottfried Wilhelm Leibniz in 1673. It was the first device capable of performing all four basic arithmetic operations directly: addition, subtraction, multiplication, and division.' }]}
                  openIndex={leibnizOpenIndex} setOpenIndex={setLeibnizOpenIndex}
                  personItemId="hist_person_leibniz" inventionBaseId="hist_inv_leibniz" onInteract={hist.trackInteraction} />

                <HistoryPersonBlock name="Joseph Jacquard" label="— French inventor of the mechanical loom, a milestone in industrialization." image={jacquardImg}
                  ...
                  inventions={[{ title: 'Jacquard Loom', image: jacquardloomImg, description: "A Jacquard loom is a revolutionary weaving machine that uses a system of interchangeable punched cards to automate the creation of intricate, complex patterns like brocade, damask, and tapestries. Invented in 1804 by French weaver Joseph-Marie Jacquard, its binary punch-card system later inspired modern computing." }]}
                  openIndex={jacquardOpenIndex} setOpenIndex={setJacquardOpenIndex}
                  personItemId="hist_person_jacquard" inventionBaseId="hist_inv_jacquard" onInteract={hist.trackInteraction} />

                <HistoryPersonBlock name="Charles Babbage" label='— English mathematician, "Father of the Computer."' image={babbageImg}
                  ...
                  inventions={[
                    { title: 'Analytical Engine', image: analyticalengineImg, description: `The Analytical Engine is widely considered the world's first conceptual design for a general-purpose, programmable digital computer. It predated modern electronic computing by a century, introducing the fundamental architecture used in computers today.` },
                    { title: 'Difference Engine', image: differenceengineImg, description: 'The Difference Engine is an early mechanical calculator designed in the 1820s by English mathematician Charles Babbage. It was built to automatically compute and tabulate polynomial functions to eliminate the severe human errors common in manual navigational and astronomical tables.' },
                  ]}
                  openIndex={babbageOpenIndex} setOpenIndex={setBabbageOpenIndex}
                  personItemId="hist_person_babbage" inventionBaseId="hist_inv_babbage" onInteract={hist.trackInteraction} />

                <HistoryPersonBlock name="Charles Xavier Thomas" label="— French inventor of the first commercially successful mechanical calculator." image={thomasImg}
                  ...
                  inventions={[{ title: 'Arithmometer', image: arithmometerImg, description: 'The arithmometer is the first commercially mass-produced mechanical calculator, patented by Charles Xavier Thomas de Colmar in 1820. It was the first reliable machine rugged enough for daily office use, significantly reducing the time taken by banks, insurance companies, and actuaries to perform math.' }]}
                  openIndex={thomasOpenIndex} setOpenIndex={setThomasOpenIndex}
                  personItemId="hist_person_thomas" inventionBaseId="hist_inv_thomas" onInteract={hist.trackInteraction} />

                <HistoryPersonBlock name="Dr. Herman Hollerith" label="— Statistician who developed the punched card tabulating system; founder of IBM's predecessor." image={hollerithImg}
                  ...
                  inventions={[{ title: 'Tabulating Machine', image: tabulatingImg, description: 'A tabulating machine was an electromechanical device designed to read, count, and sort data stored on punched cards. Invented by Herman Hollerith in the 1880s, it was famously used to process the 1890 United States Census, reducing a decade-long manual task to just one year.' }]}
                  openIndex={hollerithOpenIndex} setOpenIndex={setHollerithOpenIndex}
                  personItemId="hist_person_hollerith" inventionBaseId="hist_inv_hollerith" onInteract={hist.trackInteraction} />

                <HistoryPersonBlock name="Howard Aiken" label="— American computer pioneer who completed the Mark I in 1944." image={aikenImg}
                  ...
                  inventions={[{ title: 'Automatic Sequence Controlled Calculator (Mark I)', image: mark1Img, description: `The Automatic Sequence Controlled Calculator (ASCC), widely known as the Harvard Mark I, was one of the earliest and largest electromechanical computers. Conceived by Harvard's Dr. Howard Aiken and engineered by IBM, the machine was officially dedicated at Harvard University in 1944. It played a crucial role in World War II by calculating complex ballistics and ship designs for the U.S. Navy.` }]}
                  openIndex={aikenOpenIndex} setOpenIndex={setAikenOpenIndex}
                  personItemId="hist_person_aiken" inventionBaseId="hist_inv_aiken" onInteract={hist.trackInteraction} />

                <HistoryPersonBlock name="Dr. John Vincent Atanasoff & Clifford Berry" label="— American inventors of the first electronic computer (ABC)." image={atanasoffBerryImg}
                  ...
                  wide={true}
                  inventions={[{ title: 'Atanasoff-Berry Computer (ABC)', image: atanasoffImg, description: "The Atanasoff-Berry Computer (ABC) was the world's first automatic electronic digital computer. Developed between 1939 and 1942 at Iowa State University, it was created by physics professor John Vincent Atanasoff and graduate student Clifford Berry." }]}
                  openIndex={atanasoffOpenIndex} setOpenIndex={setAtanasoffOpenIndex}
                  personItemId="hist_person_atanasoff" inventionBaseId="hist_inv_atanasoff" onInteract={hist.trackInteraction} />

                {/* Generation of Computers — static, no tracking */}
                ...
                <div className="chap-history-block">
                  ...
                  {[
                    { period: '1951 – 1958', gen: 'First Generation', tech: 'Used vacuum tubes (about 18,000 in number, can do calculations of 10,000 additions per second)', characteristics: ['Very large', 'Expensive', 'Generated much heat'], examples: ['ENIAC'], image: gen1Img },
                    { period: '1959 – 1964', gen: 'Second Generation', tech: 'Transistor based; can perform 200,000 to 250,000 calculations per second', characteristics: ['Faster', 'Smaller', 'More reliable'], examples: [], image: gen2Img },
                    { period: '1964 – 1971', gen: 'Third Generation', tech: 'Solid state technology & integrated circuit coupled with extreme miniaturization', characteristics: ['Smaller size', 'Increased speed', 'Lower cost'], examples: [], image: gen3Img },
                    { period: '1971 – 1979', gen: 'Fourth Generation', tech: 'Increased multiprogramming and virtual storage memory (microprocessor)', characteristics: ['Personal computers emerged', 'Increased memory and storage'], examples: ['IBM PC', 'Apple Computers'], image: gen4Img },
                    { period: '1979 – Present', gen: 'Fifth Generation', tech: 'Microprocessor operating speed at 3 to 5 million calculations per second (small scale) & 10 to 15 million instructions per second (large scale)', characteristics: ['Machine Learning', 'Natural Language Processing', 'Robotics', 'Expert Systems'], examples: ['AI Assistants', 'Self-driving vehicles', 'Smart devices'], image: gen5Img },
                  ].map((gen, i) => (
                    <div key={i} className={`chap-gen-row ${i % 2 === 1 ? 'reverse' : ''}`}>
                      ...
                      <div className="chap-gen-content">
                        ...
                        <ul className="chap-gen-list">{gen.characteristics.map((c, j) => <li key={j}>{c}</li>)}</ul>
                        {gen.examples.length > 0 && (
                          <>
                            <p className="chap-gen-label">Examples</p>
                            <ul className="chap-gen-list">{gen.examples.map((e, j) => <li key={j}>{e}</li>)}</ul>
                          </>
                        )}
                      </div>
                    </div>
