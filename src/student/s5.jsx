...
// sa module na ito, naka-scroll pa rin sa dating posisyon + viewed/open state.
// sessionStorage ang gamit (hindi localStorage) para hindi dumikit sa ibang
// student na gagamit ng parehong computer.
const ALL_SECTIONS = ['software'];
const STORE_PREFIX = 'itfun:m5:';
...
function FlipCard({ frontImage, frontLabel, backText, backIcon = '💡', itemId, onInteract }) {
  ...
  const handleClick = useCallback((e) => {
    ...
    setSeen(true);
  }, [flipped, seen, onInteract, itemId]);
...
function Chapter5() {
  ...
  // ── Resume: scroll position (kinuha bago pa mag-render ang kahit ano) ──
  ...
  const [tableOpen, setTableOpen] = usePersistedState('table-open', false);
  const [tableSeen, setTableSeen] = usePersistedState('table-seen', false);
