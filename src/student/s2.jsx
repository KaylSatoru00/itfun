...
// ── Lesson totals ──
// language     : 2 accordions
// personal     : 1 flipcard
// workstation  : 1 flipcard + 1 accordion
// minicomputer : 3 accordions
const LESSON_TOTALS = {
  language:     2,
  personal:     1,
  workstation:  2,
...
function Chapter2() {
  ...
  // ── Progress trackers ──
  const langT = useProgressTracker('module2', 'lesson1', LESSON_TOTALS.language);
  const persT = useProgressTracker('module2', 'lesson2', LESSON_TOTALS.personal);
  const wsT   = useProgressTracker('module2', 'lesson3', LESSON_TOTALS.workstation);
  const miniT = useProgressTracker('module2', 'lesson4', LESSON_TOTALS.minicomputer);

  const trackerMap = { language: langT, personal: persT, workstation: wsT, minicomputer: miniT };

  const progress = {
    language:     langT.progress,
    personal:     persT.progress,
    workstation:  wsT.progress,
        ...
        <div className="chap-card-main">
          ...
          {activeSection === 'language' && (
            <>
              ...
              <div className="s2-img-wrapper">
                <img src={binaryImg} alt="Binary System" className="s2-binary-img" />
              </div>
              <div className="s2-body-block">
                <p className="s2-body-text">Binary System is the language of the computer because it composed of 2 bits that is suited to electronic device.</p>
                <p className="s2-body-text">This is because it has only two basic symbol which can represent the electrical state, it is either <strong>ON(1)</strong>, <strong>OFF(0)</strong>, either <strong>YES(1)</strong>, or <strong>NO(0)</strong>, High Low, True, False.</p>
              </div>
              <div className="chap-accordion" style={{ marginTop: 24 }}>
                {[
                  { title: 'Bit', description: 'The smallest unit of measurement in a computer. Came from the word binary digit (0 to 1).', id: 'lang_bit' },
                  { title: 'Byte', description: 'Unit of measurement that denotes one character.', id: 'lang_byte' },
                ].map((item) => (
                  <AccordionItem key={item.id} title={item.title} description={item.description} isOpen={langOpenIndex === item.id} onToggle={() => setLangOpenIndex(langOpenIndex === item.id ? null : item.id)} itemId={item.id} onInteract={langT.trackInteraction} />
                ))}
              </div>
            </>
          )}
          ...
          {activeSection === 'personal' && (
            <>
              ...
              <div className="s2-img-wrapper" style={{ marginTop: 28 }}>
                <FlipCard image={pcImg} title="PC (Personal Computer)" backText="It is a single user computer system having moderately powerful microprocessor." itemId="pers_flipcard" onInteract={persT.trackInteraction} />
              </div>
              <div className="s2-body-block" style={{ marginTop: 28 }}>
                <p className="s2-body-text">A PC can be defined as a small, relatively inexpensive computer designed for an individual user.</p>
                <p className="s2-body-text">PCs are based on the microprocessor technology that enables manufacturers to put an entire CPU on one chip.</p>
          ...
          {activeSection === 'workstation' && (
            <>
              ...
              <div className="s2-img-wrapper" style={{ marginTop: 28 }}>
                <FlipCard image={workstationImg} title="Workstation" backText="Workstation is a computer used for engineering applications (CAD/CAM), desktop publishing, software development, and other such types of applications which require a moderate amount of computing power and relatively high quality graphics capabilities." itemId="ws_flipcard" onInteract={wsT.trackInteraction} />
              </div>
          ...
          {activeSection === 'minicomputer' && (
            <>
              ...
              <div className="s2-body-block" style={{ marginTop: 16, textAlign: 'center' }}>
                <p className="s2-body-text">It is a midsize multi-processing system capable of supporting up to 250 users simultaneously.</p>
              </div>
              <div className="chap-accordion" style={{ marginTop: 16 }}>
                <AccordionItem title="Mini Computer" description="It is a multi-user computer system which is capable of supporting hundreds of users simultaneously." isOpen={miniOpenIndex === 0} onToggle={() => setMiniOpenIndex(miniOpenIndex === 0 ? null : 0)} itemId="mini_acc_mini" onInteract={miniT.trackInteraction} />
              </div>
              ...
              <div className="s2-body-block" style={{ marginTop: 16, textAlign: 'center' }}>
                <p className="s2-body-text">Mainframe is very large in size and is an expensive computer capable of supporting hundreds or even thousands of users simultaneously. Mainframe executes many programs concurrently and supports many simultaneous execution of programs.</p>
              </div>
              <div className="chap-accordion" style={{ marginTop: 16 }}>
                <AccordionItem title="Main Frame" description="It is a multi-user computer system which is capable of supporting hundreds of users simultaneously. Software technology is different from minicomputer." isOpen={miniOpenIndex === 1} onToggle={() => setMiniOpenIndex(miniOpenIndex === 1 ? null : 1)} itemId="mini_acc_main" onInteract={miniT.trackInteraction} />
              </div>
              ...
              <div className="s2-body-block" style={{ marginTop: 16, textAlign: 'center' }}>
                <p className="s2-body-text">Supercomputers are one of the fastest computers currently available. Supercomputers are very expensive and are employed for specialized applications that require immense amount of mathematical calculations (number crunching).</p>
              </div>
              <div className="chap-accordion" style={{ marginTop: 16 }}>
