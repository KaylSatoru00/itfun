...
function getStatus(progress) {
  if (progress >= 80) return { label: 'Good Standing', color: '#2e7d32', bg: '#e8f5e9' };
  if (progress >= 60) return { label: 'Warning', color: '#e65100', bg: '#fff3e0' };
  return { label: 'At Risk', color: '#c8102e', bg: '#fdecea' };
}
...
function FacultyClass() {
  ...
  const handleCreateClass = async () => {
    if (!className.trim() || !generatedCode || !user) return;
    setCreating(true);
    try {
      ...
      <AnimatePresence>
        {showCreateModal && (
          <motion.div
            className="modal-overlay"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            onClick={() => { setShowCreateModal(false); setClassName(''); setGeneratedCode(''); }}
          >
            <motion.div
            ...
            >
              ...
              <input
                ...
                placeholder="e.g. BSIT-1A SECTION A"
                value={className}
                onChange={e => setClassName(e.target.value.toUpperCase())}
                style={{ textTransform: 'uppercase' }}
                autoFocus
              />
              <button className="modal-generate-btn" onClick={handleGenerateCode}>
              ...
              <div className="modal-footer">
                <button
                  ...
                  disabled={creating || !className.trim() || !generatedCode}
                >
