...
function buildPrompt({ lessonContent, quizType, questionCount = 15 }) {
  ...
  const typeInstructions = {
    ...
    identification: `Generate EXACTLY ${questionCount} identification questions.
      ...
      - 🚫🚫 THE #1 RULE — ONE AND ONLY ONE POSSIBLE ANSWER. NEVER ask a question that has more than one valid answer. STRICTLY BANNED: "Give one example of an output device", "Name an example of X", "What is a type of X", "The ___ is an example of an output device" — when the lesson lists several members of that category (a student could answer monitor, printer, OR speaker — all correct). Instead ask for a term with ONE unique answer, e.g. "What device displays visual output on a screen?" → "Monitor". Before writing each question, ask: "Could a student give a DIFFERENT answer that is also correct?" If yes, throw it away and write a different question.
    ...
    'fill-in-blank': `Generate EXACTLY ${questionCount} fill-in-the-blank questions.
      ...
      - 🚫🚫 THE #1 RULE — ONE AND ONLY ONE POSSIBLE ANSWER. The blank must be fillable by exactly ONE word, and NO other word may also be correct. NEVER build the blank around "an example of / a type of / a kind of / a form of / one of the [category]" when the lesson lists several members of that category. ALL of the following are STRICTLY BANNED because a student could correctly answer several different words:
          ✗ "One example of an output device is _____."
          ✗ "The _____ is an example of an output device."   ← (monitor, printer, AND speaker are all correct — BANNED)
          ✗ "A _____ is a type of input device."
          ✗ "_____ is one of the application software."
        This applies no matter where the blank sits (subject or object of the sentence). Instead, make the blank a term with ONE unique answer, e.g. "The device that displays visual output on a screen is called a _____." → "Monitor". Before writing each question, ask yourself: "Could a student fill this blank with a DIFFERENT word that is also correct per the lesson?" If yes, THROW THE QUESTION AWAY and write a different one.
...
8. 🚫 THE SINGLE MOST IMPORTANT RULE — ONE AND ONLY ONE POSSIBLE ANSWER. NEVER write a question that has more than one possible valid answer. Answers are graded by EXACT match, so a question with several valid answers will mark correct students WRONG. This is the failure that has happened before, so be extremely strict about it.
   - The core test: for EVERY question, ask yourself "Could a student reasonably give a DIFFERENT answer that the lesson also supports as correct?" If YES, the question is BANNED — rewrite it so exactly one answer is possible.
   - The most common trap is asking for a MEMBER of a category when the lesson lists several members. BANNED examples (all have multiple correct answers):
       ✗ "Give ONE example of an output device" / "Name AN example of X" / "What is A type of X"
       ✗ "The _____ is an example of an output device."  (monitor, printer, AND speaker are ALL correct — BANNED)
       ✗ "One example of application software is _____."  (Word, Excel, AND PowerPoint all correct — BANNED)
     This applies in EVERY question type and no matter where the answer sits in the sentence.
   - The fix: ask for the ONE unique thing, e.g. "What device displays visual output on a screen?" → "Monitor" (only one answer), or "What type of software helps users perform tasks like word processing?" → "Application software".
   - For the SAME reason, NEVER combine multiple valid terms into a single answer using "/", "or", "and" (e.g. "Memory/Storage Unit" is WRONG — it silently packs two different valid answers into one string). Every correctAnswer must be ONE single term.
